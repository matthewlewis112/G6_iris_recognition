...
def hamming_check_string(str1,str2):
        ...
        # função que funciona apenas em python 2
        # hamming_distance_value = np.sum(np.array(map(int, str1)) != np.array(map(int, str2)))
        
        min_size = min(len(str1),len(str2))
        offset_size = abs(len(str1) - len(str2))

        hamming_distance_value += offset_size
...
def find_match(known_iris, names, iris_encodings_in_image):
        ...
        if matchlist[(matchlist.index(min(matchlist)))]<45000:
